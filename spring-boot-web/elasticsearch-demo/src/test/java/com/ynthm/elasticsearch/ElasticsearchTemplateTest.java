...
class ElasticsearchTemplateTest {
  ...
  void test() {
    ...
    IndexOperations indexOperations = elasticsearchTemplate.indexOps(Commodity.class);
  ...
  @Test
  void testInsert() {
    Iterable<Commodity> save = elasticsearchTemplate.save(CommodityFixture.DEFAULT_SUPPLIER.get());
  ...
  void testQueryAll() {
    ...
    // elasticsearchTemplate.search方法参数一,本机查询的构造,参数二index的类,可选参数三再次声明库名(可以多个)
    SearchHits<Commodity> search = elasticsearchTemplate.search(query, Commodity.class);
  ...
  @Test
  void testDeleteOne() {
    Commodity one = elasticsearchTemplate.save(CommodityFixture.ONE_SUPPLIER.get());
    String count =
        elasticsearchTemplate.delete(one.getId(), IndexCoordinates.of(IndexNameConst.COMMODITY));
  ...
  void testAgg1() {
    ...
    NativeSearchQuery query =
        nativeSearchQueryBuilder
            ...
            .build();
    SearchHits<Commodity> hits = elasticsearchTemplate.search(query, Commodity.class);
    System.out.println(hits);
    // 获取聚合结果集   因为结果为字符串类型 所以用ParsedStringTerms接收   还有ParsedLongTerms接收数字  ParsedDoubleTerms接收小数
    AggregationsContainer<?> aggregations1 = hits.getAggregations();

    //    assert aggregations1 != null;
    //    aggregations1.aggregations().
    //    ParsedStringTerms brands = aggregations1.aggregations().get("brands");
    //    // 获取桶
    //    brands
    //        .getBuckets()
    //        .forEach(
