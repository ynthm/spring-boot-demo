...
import com.ynthm.demo.mybatis.plus.enums.BusinessCode;
import com.ynthm.demo.mybatis.plus.user.entity.User;
import com.ynthm.demo.mybatis.plus.user.mapper.UserMapper;
import com.ynthm.demo.mybatis.plus.user.request.ChangePasswordRequest;
import com.ynthm.demo.mybatis.plus.user.request.ForgetPasswordRequest;
import com.ynthm.demo.mybatis.plus.user.request.RegisterRequest;
import com.ynthm.demo.mybatis.plus.user.request.UserRequest;
...
public class UserServiceImpl extends ServiceImpl<UserMapper, User> implements UserService {
  ...
  public Result<String> register(RegisterRequest request) throws Exception {
    ...
    if (StringUtils.isNotBlank(request.getEmail())) {
      ...
      agentCode = username;
      long userCount = count(Wrappers.<User>lambdaQuery().eq(User::getEmail, request.getEmail()));
      if (userCount > 0) {
        return Result.error(BaseResultCode.DB_EXIST);
      }
  ...
  public Result<Boolean> changePassword(ChangePasswordRequest request) {
    ...
    User tempUser =
        getOne(Wrappers.<User>lambdaQuery().eq(User::getUsername, request.getUsername()));
    BaseResultCode.DB_NOT_EXIST.assertNotNull(tempUser);
